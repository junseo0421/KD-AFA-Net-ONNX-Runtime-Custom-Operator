...
def benchmark_onnxruntime_gpu_hybrid(
    onnx_path,
    custom_op_dll,
    loader,
    warm=5,
    ...
    # 중요:
    # custom DLL이 Python onnxruntime-gpu 패키지의 DLL을 참조하도록 capi 경로 추가
    ort_capi_dir = pathlib.Path(ort.__file__).resolve().parent / "capi"
    os.add_dll_directory(str(ort_capi_dir))

    so = ort.SessionOptions()
    so.register_custom_ops_library(custom_op_dll)

    # profiling을 켜면 어떤 node가 어떤 provider에서 실행됐는지 확인 가능
    ...
    for batch_idx, (_, _, mask_img, name, fol) in enumerate(loader):
        ...
        fps = bs / dt if dt > 0 else float("inf")
        print(f"[ONNXRuntime-GPU-Hybrid] {batch_idx}: {dt * 1000:.3f} ms | {fps:.2f} FPS")
    ...
    print(f"\n[ONNXRuntime-GPU-Hybrid] average (after warm={warm}) : {avg_ms:.3f} ms | {avg_fps:.2f} FPS")

    profile_file = sess.end_profiling()
    print("[ORT profile saved]", profile_file)

    # provider별 node 실행 개수 요약
    try:
        ...
        print("\n[Provider summary]")
        for k, v in sorted(provider_count.items()):
            print(f"{k}: {v}")
...
def compare_torch_onnx_cuda_custom(model, onnx_path, custom_op_dll, example):
    import os
    import pathlib
    import numpy as np
    import torch
...
def benchmark_onnxruntime_cuda_custom(onnx_path, custom_op_dll, loader, warm=5):
    import os
    import time
    import json
    import pathlib
    ...
    for batch_idx, (_, _, mask_img, name, fol) in enumerate(loader):
        ...
        fps = bs / dt if dt > 0 else float("inf")
        print(f"[ONNXRuntime-CUDA-Custom] {batch_idx}: {dt * 1000:.3f} ms | {fps:.2f} FPS")
    ...
    print(f"\n[ONNXRuntime-CUDA-Custom] average (after warm={warm}) : {avg_ms:.3f} ms | {avg_fps:.2f} FPS")
...
if __name__ == "__main__":
    ...
    # benchmark_onnxruntime_gpu_hybrid(
    #     onnx_path,
    #     custom_op_dll,
    #     eval_loader,
    #     warm=warm,
    ...
    custom_op_dll_cuda = r"C:\Users\8138\PycharmProjects\DION4FR_student_test\custom_ops\kdafa_ort_cuda\build\Release\kdafa_custom_ops_cuda.dll"

    compare_torch_onnx_cuda_custom(
        export_model,
        onnx_path,
        custom_op_dll_cuda,
        example
    ...
    benchmark_onnxruntime_cuda_custom(
        onnx_path,
        custom_op_dll_cuda,
        eval_loader,
        warm=warm
